function [startRow,endRow] = fname2fresh(X,MAX_X,filename)
...
formatSpec = '%5s%4s%6s%6s%6s%7s%7s%7s%6s%[^\n\r]';
...
Fresh = [dataArray{1:end-1}];
run=~cellfun('isempty',strfind(cellstr(Fresh),'*RUN'));
run_loc=find(run);
integerTest=~mod(X,1);
